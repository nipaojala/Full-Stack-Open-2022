  ...
  return (
    <div>
      ...
      <form onSubmit={handleSubmit}>
        ...
        <button onClick={(event) => resetValues(event)}>reset</button>
      </form>
    </div>
  )
...
const ShowSingleAnecdote = ({anecdote}) => {
  return (
    <div>
...
const App = () => {
  ...
  const addNew = (content, author, info) => {
    const votes = 0
    const anecdote = {
    ...
    <div>
      ...
      <Notification setNotification={setNotification} notification={notification}/>
        <Routes>
          ...
          <Route path='/anecdotes/:id' element={<ShowSingleAnecdote anecdote={anecdote}/>}/>
          <Route path='/create' element={<CreateNew new={addNew}/>} />
